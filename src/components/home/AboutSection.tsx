...
export const AboutSection: React.FC<AboutSectionProps> = ({ profile, achievements }) => {
  ...
  return (
    <section id="about" className="scroll-mt-28 py-20 bg-white border-b border-slate-200/80">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-10 items-start">
            ...
            <div className="pt-4">
              ...
              <div className="flex flex-wrap gap-2">
                {profile.interests[language].map((interest, idx) => (
                  <span
                    key={idx}
                    className="inline-flex items-center gap-1.5 px-3 py-1.5 rounded-lg bg-slate-50 text-slate-700 text-xs font-semibold border border-slate-200 hover:border-indigo-300 hover:bg-indigo-50/50 transition-colors"
                  >
          ...
          <div className="lg:col-span-5 space-y-4">
            <div className="bg-slate-50/80 p-6 rounded-2xl border border-slate-200/90 shadow-xs">
              ...
              <div className="space-y-4">
                {achievements.map((item) => (
                  <div
                    key={item.id}
                    className="p-3.5 rounded-xl bg-white border border-slate-200/70 hover:border-indigo-300 transition-all shadow-xs"
                  >
                    <div className="flex items-start justify-between gap-2">
                      ...
                      {item.badge && (
                        <span className="shrink-0 text-[10px] uppercase font-extrabold px-2 py-0.5 rounded-full bg-indigo-50 text-indigo-700 border border-indigo-200">
                          {item.badge}
                        </span>
                      )}
                    </div>
